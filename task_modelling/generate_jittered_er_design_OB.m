function [meanrecipvif, vifs, design_struct] = generate_jittered_er_design_OB(varargin)
...
trialtypes = 13;        % neutral, 2 levels of loss, 2 levels of gain
trialspertype = 20;
...
ISImin = 0.7;           % Constraints: Psychological (can subjects process cue) and statistical (longer = less BOLD nonlinearity, which is difficult to model).
ISImean = 2.5;            % For 'exponential' only.  Includes ISImin.  There is an optimal empirical value -- longer is better for deconvolution/FIR, but we also need to fit within total scan time constraints.
...
ISImax = 5.5;             % Truncate to avoid VERY long ISIs
