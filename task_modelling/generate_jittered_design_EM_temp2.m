function [meanrecipvif, vifs, design_struct] = generate_jittered_design_EM_temp2(varargin)
...
event1duration = 4.5;    % duration of cue
trialtypes = 5;         % stop, go
trialspertype = [20 20 20 20 10]; % for one run; 300 go, 60 stop total

% All ISI times in sec.
isidistribution = 'exponential';  % 'exponential' or 'geometric'
ISImin = 3;           % Constraints: Psychological (can subjects process cue) and statistical (longer = less BOLD nonlinearity, which is difficult to model).
ISImean = 4.5;          % For 'exponential' only.  Includes ISImin.  There is an optimal empirical value -- longer is better for deconvolution/FIR, but we also need to fit within total scan time constraints.
...
ISImax = 7;             % Truncate to avoid VERY long ISIs

allow_repeats = [1 1 1 1 1];  % Repeats ok for each trial type?  1=go=yes, 2=stop=no
