...
path_save=['/Users/yeojin/Dropbox/literatures_IKND/REPLAY/stim/stiminfo/altmodel2/'];
...
for ii = 3:4 % number of unique matrices you'd like to create
    ...
    while cnt == errs % try until it doesn't error
        try
            ...
            [meanrecipvif, vifs, design_struct] = generate_jittered_design_REPLAY_BLK1; % run the specific GA optimisation function
            
            % save diagnostics figure
            fig = gcf;
            fig.PaperPositionMode = 'auto';
            print(['GA_REPLAY_RET7T_block' num2str(ii) '_Diagnostics'],'-dpng','-r0')
            close figure 1
            
            % save predicted HRF function figure
            fig = gcf;
            fig.PaperPositionMode = 'auto';
            print(['GA_REPLAY_RET7T_block' num2str(ii) '_PredictedActivity'],'-dpng','-r0')
            close figure 2
            
            % tailor the design matrix to your task
            tmp = design_struct.eventlist(:,2);
            tmp2(find(tmp==1),1)= 2;
%             tmp2(find(tmp==2),1)= 2;
            tmp2(find(tmp==2),1)= 3;
            tmp2(find(tmp==3),1)= 0;
            
            design_struct.eventlist(:,2) = tmp2; % assign the new regime
            
            % save the design matrix and efficiency infos
            save([path_save '/REPLAY_RET7T_GA_designstruct_block' num2str(ii) '.mat' ],...
                'design_struct','meanrecipvif','vifs')
            keep ii cnt errs path_save
