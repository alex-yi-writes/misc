function [meanrecipvif, vifs, design_struct] = generate_jittered_design_EM_temp(varargin)
...
trialtypes = 3;         % stop, go
trialspertype = [45 45 10]; % for one run; 300 go, 60 stop total
...
allow_repeats = [1 1 0];  % Repeats ok for each trial type?  1=go=yes, 2=stop=no
