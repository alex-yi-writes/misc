function [meanrecipvif, vifs, design_struct] = generate_jittered_design_REPLAY_BLK1(varargin)
...
trialtypes = 3;         % cue1, control, null
trialspertype = [20 10 2]; % for one run; 300 go, 60 stop total

% All ISI times in sec.
isidistribution = 'exponential';  % 'exponential' or 'geometric'
ISImin = 1;           % Constraints: Psychological (can subjects process cue) and statistical (longer = less BOLD nonlinearity, which is difficult to model).
...
ISImax = 4;             % Truncate to avoid VERY long ISIs

allow_repeats = [1 1 1];  % Repeats ok for each trial type?  1=go=yes, 2=stop=no
